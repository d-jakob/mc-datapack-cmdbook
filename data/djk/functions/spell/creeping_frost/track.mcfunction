execute as @e[type=minecraft:armor_stand, tag=creeping_frost] at @s run tp @s @e[type=minecraft:snowball, tag=tracked, nbt={Item:{tag:{creeping_frost:1}}}, limit=1, sort=nearest, distance=..2]
